 ...
 *
 * <p>Mixing touch input and gyro input results in NetWorkUtil complicated UI so this should be used
 * carefully. This touch system implements NetWorkUtil basic (X, Y) -> (yaw, pitch) transform. This works for
 ...
 * NetWorkUtil nicer UI. An even more advanced UI would reproject the user's touch point into 3D and drag the
...
/* package */ class TouchTracker extends GestureDetector.SimpleOnGestureListener
        implements View.OnTouchListener, OrientationListener.Listener, ScaleGestureDetector.OnScaleGestureListener {
...
//        Log.e("MyLog", "scaleFactor >>> " + scaleFactor);

        totalScale = totalScale / scaleFactor;

        if (totalScale >= 1.0f) {
            totalScale = 1.0f;
        } else if (totalScale <= 0.4f) {
            totalScale = 0.4f;
        }

//        Log.e("MyLog", "totalScale >>> " + totalScale);

        if (listener != null) {
            listener.onScale(totalScale);
        }
...
//        mScaleGestureDetector.setQuickScaleEnabled(false);
    ...
    private boolean isStartScaleHandle;
...
//            Log.e("MyLog", " actionMasked >>> " + actionMasked);
            if (actionMasked == MotionEvent.ACTION_DOWN) {
                downTime = System.currentTimeMillis();
            } else if (actionMasked == MotionEvent.ACTION_UP) {
                long tapTime = System.currentTimeMillis() - downTime;
                isTapTimeOut = tapTime >= TAP_TIMEOUT;
                isStartScaleHandle = false;
            } else if (actionMasked == MotionEvent.ACTION_POINTER_DOWN) {
                isStartScaleHandle = true;
                downTime -= 2000;
            } else if (actionMasked == MotionEvent.ACTION_POINTER_UP) {
                isStartScaleHandle = false;
            }

            if (isStartScaleHandle) {
...
//        accumulatedTouchOffsetDegrees.x = 0;
    ...
    public boolean onSingleTapUp(MotionEvent e) {
        if (singleTapListener != null && !isTapTimeOut) {
    ...
    public void clearTouch() {
        previousTouchPointPx.x = 0;
        previousTouchPointPx.y = 0;
        accumulatedTouchOffsetDegrees.x = 0;
        accumulatedTouchOffsetDegrees.y = 0;
    }
    ...
    public void restoreGLStatus(SphericalGLSurfaceView.GlStatusHolder glStatusHolder) {
        previousTouchPointPx.x = glStatusHolder.previousTouchPx.x;
        previousTouchPointPx.y = glStatusHolder.previousTouchPx.y;
        accumulatedTouchOffsetDegrees.x = glStatusHolder.accumulatedTouchOffset.x;
        accumulatedTouchOffsetDegrees.y = glStatusHolder.accumulatedTouchOffset.y;

        totalScale = glStatusHolder.totalScale;
    }

    public void saveGLStatus(SphericalGLSurfaceView.GlStatusHolder glStatusHolder) {
        glStatusHolder.accumulatedTouchOffset.x = accumulatedTouchOffsetDegrees.x;
        glStatusHolder.accumulatedTouchOffset.y = accumulatedTouchOffsetDegrees.y;
        glStatusHolder.previousTouchPx.x = previousTouchPointPx.x;
        glStatusHolder.previousTouchPx.y = previousTouchPointPx.y;

        glStatusHolder.totalScale = totalScale;
    }
}
