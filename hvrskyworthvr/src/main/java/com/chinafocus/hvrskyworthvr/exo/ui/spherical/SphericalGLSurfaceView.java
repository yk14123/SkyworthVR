 ...
 */
package com.chinafocus.hvrskyworthvr.exo.ui.spherical;
...
/**
 * Renders NetWorkUtil GL scene in NetWorkUtil non-VR Activity that is affected by phone orientation and touch input.
 *
 * <p>The two input components are the TYPE_GAME_ROTATION_VECTOR Sensor and NetWorkUtil TouchListener. The GL
 * renderer combines these two inputs to render NetWorkUtil scene with the appropriate camera orientation.
 *
...
public final class SphericalGLSurfaceView extends GLSurfaceView {
    ...
    private static float FIELD_OF_VIEW_DEGREES = 90;
    ...
    public SphericalGLSurfaceView(Context context, @Nullable AttributeSet attributeSet) {
        ...
        touchTracker = new TouchTracker(context, mRenderer, PX_PER_DEGREES);
        WindowManager windowManager = (WindowManager) context.getSystemService(Context.WINDOW_SERVICE);
        Display display = Assertions.checkNotNull(windowManager).getDefaultDisplay();
        orientationListener = new OrientationListener(display, touchTracker, mRenderer);
    ...
    public void resetVR() {
        ...
        mRenderer.resetScale();
        touchTracker.clearScale();
    }
    ...
    public void saveGLStatus(GlStatusHolder glStatusHolder) {
        mRenderer.saveGLStatus(glStatusHolder);
        touchTracker.saveGLStatus(glStatusHolder);
    }

    public void restoreGLStatus(GlStatusHolder glStatusHolder) {
        mRenderer.restoreGLStatus(glStatusHolder);
        touchTracker.restoreGLStatus(glStatusHolder);
    }

    /**
     * Sets the default stereo mode. If the played video doesn't contain NetWorkUtil stereo mode the default one
     * is used.
     *
...
//            deviceRoll = UPRIGHT_ROLL;
            ...
            scrollDegreesX = 0;
            updatePitchMatrix();
            Matrix.setRotateM(touchYawMatrix, 0, 0, 0, 1, 0);
        }

        private float scrollDegreesX;

        public void saveGLStatus(GlStatusHolder glStatusHolder) {
            glStatusHolder.pitch = touchPitch;
            glStatusHolder.scrollOffset = scrollDegreesX;
            glStatusHolder.VIEW_DEGREES = FIELD_OF_VIEW_DEGREES;
        }

        public void restoreGLStatus(GlStatusHolder glStatusHolder) {
            touchPitch = glStatusHolder.pitch;
            updatePitchMatrix();
            scrollDegreesX = glStatusHolder.scrollOffset;
            Matrix.setRotateM(touchYawMatrix, 0, glStatusHolder.scrollOffset, 0, 1, 0);

            FIELD_OF_VIEW_DEGREES = glStatusHolder.VIEW_DEGREES;
            float fovY = calculateFieldOfViewInYDirection(mAspect);
            Matrix.perspectiveM(projectionMatrix, 0, fovY, mAspect, Z_NEAR, Z_FAR);
        }
        ...
        /**
         * Updates the pitch matrix after NetWorkUtil physical rotation or touch input. The pitch matrix rotation
        ...
        public synchronized void onScrollChange(PointF scrollOffsetDegrees) {
            ...
            scrollDegreesX = -scrollOffsetDegrees.x;

            Matrix.setRotateM(touchYawMatrix, 0, scrollDegreesX, 0, 1, 0);
            // 上面这一句，切换成下面这一句，就没有X方向平移了
//            Matrix.setRotateM(touchYawMatrix, 0, 0, 0, 1, 0);
        }
        ...
        public void onScale(float scaleFactor) {
            synchronized (this) {
//                FIELD_OF_VIEW_DEGREES = (float) ((scaleFactor - 0.25) / (1 - 0.25) * 30);
                FIELD_OF_VIEW_DEGREES = ((scaleFactor - 0.4f) * 40.0f / 0.6f) + 50.0f;
                float fovY = calculateFieldOfViewInYDirection(mAspect);
                Matrix.perspectiveM(projectionMatrix, 0, fovY, mAspect, Z_NEAR, Z_FAR);
            }
        }
        ...
        public void resetScale() {
            FIELD_OF_VIEW_DEGREES = 90.f;
            float fovY = calculateFieldOfViewInYDirection(mAspect);
            Matrix.perspectiveM(projectionMatrix, 0, fovY, mAspect, Z_NEAR, Z_FAR);
        }
    }

    public static class GlStatusHolder {
        public float pitch;
        public float scrollOffset;
        public float VIEW_DEGREES;
        public PointF previousTouchPx = new PointF();
        public PointF accumulatedTouchOffset = new PointF();
